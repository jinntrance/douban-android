...
class BookActivity extends DoubanActivity {
  ...
  protected override def onCreate(b: Bundle) {
    ...
    book =if(book.isDefined) book else extras match {
      case x if !extras.getString(Constant.ISBN).isEmpty => Some(Book.byISBN(extras.getString(Constant.ISBN)): Book)
      case x if !extras.getString(Constant.BOOK_ID).isEmpty => Some(Book.byId(extras.getString(Constant.BOOK_ID).toLong))
      case x if SearchResult.selectedBook.isDefined => SearchResult.selectedBook
      case _ => {
        this.finish()
        None
      }
